...
role=os.environ['ROLE']; model=os.environ['MODEL']; focus=os.environ.get('FOCUS','general physics')
mission=pathlib.Path('mission/SCIENCE-PHYSICS-MISSION.md').read_text()
prompt=f'''You are role {role} in CEREBRON Farm 10 Science Physics.\nFocus: {focus}\n{mission}\nProduce rigorous physics analysis. Define symbols and units, check dimensions, state assumptions and validity domain, separate ESTABLISHED/DERIVED/MODEL/SPECULATIVE/UNKNOWN, seek falsification, and never claim experimental validation without evidence.'''
...
out={'role':role,'model':model,'focus':focus,'success':ok,'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','result':text if ok else None,'meta':meta}
